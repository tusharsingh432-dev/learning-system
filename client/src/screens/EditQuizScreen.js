...
export default function EditQuizScreen({ match }) {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                const response = await axios.post('/api/quiz/getById', { quizId: match.params.quizid });
                setCourseId(response.data.courseId);
                setTitle(response.data.name);
                setTotalMarks(response.data.totalMarks);
                setPassingMarks(response.data.passingMarks);
                const ques = await axios.post('/api/question/getByQuizId', { quizId: match.params.quizid });
    ...
    function handleCreateQuiz() {
        ...
        // console.log({ quiz, questions, courseId })
        dispatch(editQuiz({ quiz, questions, courseId }));
    }
    ...
    return <div>
        <div className="row justify-content-center mt-5">
            <div className="col-md-5 mt-5 shadow p-3 mb-5 bg-white rounded">
                ...
                <button className="btn btn-primary" type="submit" onClick={handleCreateQuiz}>Update Quiz</button>
